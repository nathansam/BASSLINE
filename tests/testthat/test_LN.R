...
test_that("Prior_LN returns expect values", {
  ...
  for (prior in 1:3) {
    for (logs in c(T, F)) {
      expect_equivalent(
        prior_LN(betas, sigma2, prior, logs),
...
test_that("Prior_LN returns expected value for prior = 2", {
  ...
  p <- 1
  aux <- -p * log(sigma2)
  expect_equivalent(prior_LN(
    beta = c(4, 1, 4), sigma2 = 0.1, prior = 2,
    logs = T
  ), aux)
})
...
test_that("logt_update_SMLN same in C++ as in R", {
  ...
  set.seed(123)
  result.R <- logt.update.SMLN(
    Time, Cens, X, beta, sigma2, set, eps_l,
    eps_r
  )
  set.seed(123)
  result.Cpp <- logt_update_SMLN(
    Time, Cens, X, beta, sigma2, set, eps_l,
    eps_r
  )
