...
to separate variables for each level. For example:
...
```{r Converted Table, echo = F}
### RUN FOR EXAMPLE TABLE ###
df.2 <- df
n.obs <- nrow(df.2)

treatment.A <- rep(0, n.obs)
treatment.B <- rep(0, n.obs)
treatment.C <- rep(0, n.obs)

for(i in 1:n.obs){
  if(df.2$treatment[i] == "A") treatment.A[i] <- 1
  if(df.2$treatment[i] == "B") treatment.B[i] <- 1 
  if(df.2$treatment[i] == "C") treatment.C[i] <- 1
}

df.2 <- cbind(df.2, treatment.A, treatment.B, treatment.C)
df.2$treatment <- NULL

mat.2 <- as.matrix(df.2)
knitr::kable(mat.2)
```

Which can be obtained by running the following R code
...
for(i in 1:n.obs){
  if(df$treatment[i] == "A") treatment.A[i] <- 1
  if(df$treatment[i] == "B") treatment.B[i] <- 1
  if(df$treatment[i] == "C") treatment.C[i] <- 1
}
df <- cbind(df, treatment.A, treatment.B, treatment.C)
df$treatment <- NULL
mat <- as.matrix(df)
```
...
plot a chain for an observation across (non-discarded) iterations. We will 
investigate the chain for the first observation from the log-normal model. 

```{r Trace Plot, fig.retina=2}
Trace_Plot(1, LN)
```

In this case, the plot provides evidence for using an increased burn-in period
due to it's profile for the first few iterations. 

The user is also advised to plot the auto-correlation function which can be done
via the `forecast` and `ggplot2` packages.

```{r ACF plot, message = F, fig.retina= 2}
library(ggplot2)
library(forecast)
p <- ggAcf(LN[1,]) + ggplot2::theme_bw()
p <- p + ggplot2::ggtitle("ACF of MCMC Chain for First Observation")
p
```
